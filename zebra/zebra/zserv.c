...
/*  display olp network information */
DEFUN (show_olp_network,
       show_olp_network_cmd,
       "show network",
       SHOW_STR
       "show olp network information\n")
{
#if 0
  STATUS  status;
  UINT32     address;
  ...
  status = get_ip_address_ipc_lib(&address);
  inet_ntop(AF_INET, (void *)&address, str_address, 16);
   if (status == OK)
   	 vty_out (vty, "OLP ip address :%s\r\n", str_address);
  else
    	vty_out (vty, "get data fail status:%d\r\n", status);
#endif
...
zebra_init ()
{
  ...
  //install_element (VIEW_NODE, &show_olp_network_cmd);
  //install_element (ENABLE_NODE, &show_ip_forwarding_cmd);
  //install_element (ENABLE_NODE, &show_olp_network_cmd);
