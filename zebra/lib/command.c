/* Command interpreter routine for virtual device [aka TeletYpe]
...
char *default_motd = 
"\r\n\
Copyright 2010-2014 %s.\r\n\
\r\n";

/* Standard command node structures. */
struct cmd_node login_node = 
{
  LOGIN_NODE,
  "Login: ",
};
...
config_write_host (struct vty *vty)
{
  ...
  if (host.lines >= 0)
    vty_out (vty, "service device-length %d%s", host.lines,
	     VTY_NEWLINE);
       ...
       "Turn on enable mode command\n")
{
	...
	{
		vty_out ( vty,
				"Internal error! Enter enable mode failed!%s",
				VTY_NEWLINE );
		return CMD_WARNING;
	}

	/* prepare to enter enable mode */
	if ( ( E_PRIVI_ADMIN > stUserInfo.privilege )
		|| ( E_PRIVI_MANAGER < stUserInfo.privilege ) )
	{
		vty_out ( vty,
...
DEFUN (config_write_file, 
       ...
       "write file",  
       "Write running configuration to memory, network, or device\n"
...
/* Write current configuration into the device. */
DEFUN (config_write_device,
       config_write_device_cmd,
       "write device",
       "Write running configuration to memory, network, or device\n"
       "Write to device\n")
{
...
/* Write current configuration into the device. */
ALIAS (config_write_device,
       show_running_config_cmd,
       "show running-config",
       SHOW_STR
       "running configuration\n");

/* Write startup configuration into the device. */
       ...
       "This system's network name\n")
{
  ...
  #if 0
	STATUS status = OK;
	status = set_system_name_ipc_lib(host.name);
    if (status != OK) {
      	vty_out (vty, "Failed to set host name%s", VTY_NEWLINE);
    }
    else
    {
      	vty_out (vty, "Set host name %s successfully%s", host.name,VTY_NEWLINE);
    }
  #endif
  return CMD_SUCCESS;
}
...
DEFUN (config_password, password_cmd,
       "password (8|) WORD",
       "Assign the device connection password\n"
...
DEFUN (service_terminal_length, service_terminal_length_cmd,
       "service device-length <0-512>",
       "Set up miscellaneous service\n"
       "System wide device length configuration\n"
...
DEFUN (no_service_terminal_length, no_service_terminal_length_cmd,
       "no service device-length [<0-512>]",
       NO_STR
       "Set up miscellaneous service\n"
       "System wide device length configuration\n"
       "Number of lines of VTY (0 means no line control)\n")
