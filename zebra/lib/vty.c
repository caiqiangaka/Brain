  ...
  if (ret != 0)
    {
      ...
      tmp = write (vty->fd, buf, strlen (time_buf) + 1);
    }

  snprintf (buf, sizeof buf, "%s: ", proto_str);
  tmp = write (vty->fd, buf, strlen (proto_str) + 2);

  len = vsnprintf (buf, sizeof buf, format, va);
  if (len < 0)
    return -1;
  tmp = write (vty->fd, (u_char *)buf, len);

  snprintf (buf, sizeof buf, "\r\n");
  tmp = write (vty->fd, buf, 2);
...
static void
vty_user_login (struct vty *vty)
{
	vty->node = VIEW_NODE; /* enter view mode */
#if 0
	STATUS status = OK;
	int fail = 0;
	char username[CHECK_USER_NAME_SIZE];

	/* check user name's length */
	if ( CHECK_USER_NAME_SIZE - 1 < vty->length )
	{
		fail = 1;
	}
	else
	{
		/* call ipc to check user name */
		memset( username, '\0', sizeof( username ) );
		memcpy( username, vty->buf, vty->length * sizeof( char ) );
		status = set_check_user_name_ipc_lib( username );
		if ( OK != status )
		{
			fail = 1;
		}
	...
	/* check user name successfully */
	if ( !fail )
	{
		vty->fail = 0;
		vty->user = strdup( vty->buf );
		vty->node = AUTH_NODE;
		return;
	}
	...
	{
		vty_out ( vty, 
				"%% Bad user name, too many failures!%s", 
				VTY_NEWLINE );
...
/* Authentication of vty */
static void
vty_auth_password (struct vty *vty)
{
	vty->node = VIEW_NODE; /* enter view mode */
	...
	int fail = 0;
	STATUS status = OK;
	CHECK_PASSWORD_T stPasswd;

	/* check password's length */
	if ( sizeof( stPasswd.password ) - 1 < vty->length * sizeof( char ) )
	{
		fail = 1;
	}
	else
	{
		memset( &stPasswd, '\0', CHECK_PASSWORD_SIZE );
		strcpy( stPasswd.name, vty->user );
		memcpy( stPasswd.password, vty->buf, vty->length * sizeof( char ) );
		status = set_check_password_ipc_lib( &stPasswd );
		if ( OK != status )
		{
			fail = 1;
		}
...
vty_write (struct vty *vty, char *buf, size_t nbytes)
{
  if ((vty->node == AUTH_NODE) 
	...
	|| (vty->node == AUTH_ADMIN_NODE)
	|| (vty->node == ENTER_PASSWD_NODE)
	|| (vty->node == TTY_SWITCH_NODE))
    return;
...
vty_end_config (struct vty *vty)
{
  ...
  switch (vty->node)
    ...
    case ENABLE_NODE:
      ...
      break;
	case ADMIN_NODE:
	  vty_admin_unlock (vty);
	  vty->node = vty->returnto;
	  break;
    case CONFIG_NODE:
    case INTERFACE_NODE:
    case ZEBRA_NODE:
...
vty_stop_input (struct vty *vty)
{
  ...
  switch (vty->node)
	...
	case ADMIN_NODE:
	  ...
	  vty->node = vty->returnto;
	  break;
	case ENTER_USER_NODE:
	case ENTER_PASSWD_NODE:
...
vty_execute (struct vty *vty)
{
  ...
  switch (vty->node)
    ...
    case LOGIN_NODE:
      ...
      break;
    case AUTH_NODE:
	  vty_auth_password (vty);
	  break;
    case AUTH_ENABLE_NODE:
    case AUTH_ADMIN_NODE:
      break;
	case ENTER_USER_NODE:
	case ENTER_PASSWD_NODE:
	case TTY_SWITCH_NODE:
	  ret = vty->func(vty, NULL);
	  if (CMD_SUCCESS != ret && NULL != vty->index)
	  {
		  XFREE(MTYPE_TMP, vty->index);
		  vty->index = NULL;
	  }
...
void
vty_close_by_id(struct vty *vty, int id)
{
#if 0
	struct vty *v;
	STATUS status = OK;
	USER_TARGET_INFO_T stUserInfo;

	v = vector_slot ( vtyvec, id );
	if ( NULL == v )
	{
		vty_out ( vty, 
				"vty[%d] is not exist, close fail!%s",
				id, VTY_NEWLINE );
		return;
	}
	...
	if ( v == vty )
	{
		vty_out ( vty, "can't close yourself, close failed!%s",
				VTY_NEWLINE );
		return;
	}

	/* get privilege */
	memset( &stUserInfo, '\0', USER_TARGET_INFO_SIZE );
	strcpy( stUserInfo.name, vty->user );
	status = get_user_target_info_ipc_lib( &stUserInfo );
	if ( OK != status )
	{
		vty_out ( vty,
				"Internal error! close vty failed!%s",
				VTY_NEWLINE );
		return;
	}

	if ( ( 0 != strcmp( vty->user, v->user ) )
		&& ( E_PRIVI_ADMIN != stUserInfo.privilege ) )
	{
		vty_out ( vty,
				"Permission denied! Please contact your administrator!%s",
				VTY_NEWLINE );
		return;
	}

	vty_out (vty, "vty[%d] connected from %s by user %s will be closed by user %s!%s",
			id, v->address, v->user, vty->user, VTY_NEWLINE);
	vty_close(v);
#endif
}
...
int
vty_admin_unlock (struct vty *vty)
{
	if (vty_admin == 1 && vty->admin == 1)
	{
		vty->admin = 0;
		vty_admin = 0;
	}
...
#ifdef OSL6200
int
vty_stop_monitor (int slotid)
{
	int i = 0;
	int stopNum = 0;
	struct thread stThread;
	struct vty *v;

	for ( i = 0; i < vector_max( vtyvec ); i++ )
	{
		v = vector_slot( vtyvec, i );
		if ( NULL == v )
		{
			continue;
		}
		...
		if ( ( slotid == v->slotid )
			&& ( 1 == v->monitor_flag ) )
		{
			v->monitor_flag = 0;  // stop monitor
			v->slotid       = -1; // monitor none
			v->node         = v->returnto; // turn off current mode, and return to pre mode
			stopNum++; // how many users will be stopped monitor

			// clear command line buffer
			v->cp = v->length = 0;
			vty_clear_buf( v );

			// auto print a new prompt
			if ( ( VTY_CLOSE != v->status )
				&& ( VTY_START != v->status )
				&& ( VTY_CONTINUE != v->status ) )
			{
				memset( &stThread, 0, sizeof( stThread ) );
				stThread.arg = v;
				stThread.u.fd = v->fd;

				vty_out( v, "\r\nThe slot card has been pulled out!\r\n");
				vty_prompt( v );
				vty_flush( &stThread );
			}
		}
       ...
       "Display who is on vty\n")
{
	...
	for (i = 0; i < vector_max (vtyvec); i++)
	{
		...
		{
			vty_out ( vty, "%-5d", cnt + 1 );
			vty_out ( vty, "%s%04d%s ", "vty[", i, "]" );
			vty_out ( vty, "%-16s", v->address );
			if ( v->config )
			{
				vty_out ( vty, "%-10s", "config" );
			}
			else if ( v->admin )
			{
				vty_out ( vty, "%-10s", "admin" );
			}
			else
			{
				vty_out ( vty, "%-10s", "-" );
			}
			vty_out ( vty, "%s%s", v->user, VTY_NEWLINE );
			cnt++;
		}
	}
...
DEFUN (terminal_monitor,
       ...
       "Copy debug output to the current device line\n")
{
  vty->monitor = 1;
  return CMD_SUCCESS;
}
